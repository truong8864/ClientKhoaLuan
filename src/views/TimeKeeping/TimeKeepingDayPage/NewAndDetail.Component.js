...
const NewAndDetail = (props) => {
  ...
  const handleOnClose = () => {
    if (Modifile) return setConfim(true);
    show(null);
  };

  const onSave = async () => {
    if (!Document.CodeEmp || Err.CodeEmp || Err.TimeIn || Err.TimeOut)
      return alert("Dữ liệu còn thiếu hoặc bị lỗi");
    if ("update" !== Option) {
    ...
    await DayKeepingAPI.update(_id, data);
              ...
              <TextField
                ...
                onBlur={async () => {
                  if (!Document.CodeEmp) {
                    return setErr({
                      ...Err,
                      CodeEmp: "Chưa nhập mã nhân viên",
                    });
                  }
                  const result = await ProfileAPI.getByCodeEmp(
                    Document.CodeEmp
                  );
                  if (!result.data) {
                    return setErr({
                      ...Err,
                      CodeEmp: "Mã nhân viên không chính xác",
                    });
                  }
                  ...
                  setDocument({
                    ...
                    CodeAttendance: result.data.CodeAttendance,
                  });
                }}
                onChange={(event) => {
              ...
              <Grid item xs={2}>
                <FormControl error={Err.TimeIn !== null} fullWidth>
                  ...
                  <KeyboardTimePicker
                    ...
                    onChange={(date) => {
                      const check =
                        new Date(date) >= new Date(Document.TimeOut);
                      if (check) {
                        return setErr({
                          ...Err,
                          TimeIn: "GIO VAO K THE LON HON HOAC BANG GIO RA",
                        });
                      }
                  ...
                  <KeyboardTimePicker
                    ...
                    onChange={(date) => {
                      ...
                      if (check) {
                        return setErr({
                          ...Err,
                          TimeOut: "GIO RA K THE NHO HON HOAC BANG GIO VAO",
                        });
                      }
                      ...
                      <TextField
                        value={
                          !Document.Total
                            ? ""
                            : Math.ceil(
                                (parseInt(Document.Total) / (1000 * 60 * 60)) *
                                  10
                              ) / 10
                        }
                        disabled
