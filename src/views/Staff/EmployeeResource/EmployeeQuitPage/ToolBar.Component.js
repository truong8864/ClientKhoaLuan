...
const ToolBar = (props) => {
  ...
  const Approve = async () => {
    try {
      const data = await StopWorkingAPI.update(RowSelected._id, {
        Status: "E_APPROVED",
      });
      onSearch();
    } catch (error) {
    ...
    <Toolbar variant="dense" disableGutters className={classes.root}>
      ...
      <div className={classes.right}>
        <div>
          ...
          <IconButton onClick={AddBlackList}>
            <Tooltip title="Thêm vào danh sách đen">
              <CancelIcon />
            </Tooltip>
          </IconButton>
          {
          ...
          <IconButton
            disabled={
              JSON.stringify(RowSelected) === JSON.stringify({}) ? true : false
            }
            onClick={Detail}
          >
